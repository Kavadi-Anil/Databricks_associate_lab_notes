...
-- MAGIC The challenge is optional during a live class if you have time. This challenge my require you to use documentation. If you run out of time during a live class, try and complete this after class.
-- MAGIC
-- MAGIC #### Optional Challenge Scenario
-- MAGIC You report back to your data team and everyone agrees to clean up any values in the rescued data column that contain a `$` when ingesting as a bronze table. To fix this issue, you agree to handle these edges case during ingestion by leveraging the `_rescued_data` column.
-- MAGIC
-- MAGIC Your team has decided to strip the `$` in the price and simply store the numeric vaue during ingestion.
-- MAGIC
-- MAGIC
-- MAGIC #### Requirements
-- MAGIC
